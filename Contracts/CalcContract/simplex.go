...
func SimplexMax(c []float64, A mat.Matrix, b []float64, tol float64, initialBasic []int) (optF float64, optX []float64, err error) {
...
func simplex(initialBasic []int, c []float64, A mat.Matrix, b []float64, tol float64) (float64, []float64, []int, error) {
	...
	for {
        ...
        // if max is below than zero, all r are negative
		if r[maxIdx] <= -tol {
			break
		}
