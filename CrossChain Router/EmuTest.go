...
func SendCrossChainRequestTest(SCID, TCID, CalcType string) error {
	...
	var aesKeyEncoded string
	keyinfo := KeyInfo{TCID: TCID, UUID: uu}
	err = conn.Call("RpcServer.GenerateKey", keyinfo, &aesKeyEncoded)
	if err != nil {
		log.Println(err)
	}
	// set local key
	LocalKey = Base58Decoding(aesKeyEncoded)
	jsbytes1, err1 := json.Marshal(msg)
	if err1 != nil {
		log.Println("fail to marshal the msg")
		return err1
	}
	encMsg, err2 := AesEncrypt(jsbytes1, LocalKey)
	if err2 != nil {
		log.Println("fail to encrypt the msg")
		return err2
	}
